...
const MakeAppointmentPage = () => {
  ...
  //get department doctor without token
  const departmentDoctor = async () => {
    console.log('get department doctor')
  ...
  const onSubmit = async () => {
    if (getValues('department_id') === 0) {
      ...
      console.log(getValues('doctor_id'))
      const payload = {
        name: getValues('name'),
      ...
      if (data.status === 1552) {
        ...
        setAlertPrompt(true)
      } else if (data.status === 1558) {
        presentAlert({
          header: 'Alert',
          subHeader: `Let's register a new account!`,
          buttons: [
            {
              text: 'ok',
              handler: () => {
                    ...
                    <IonSelect onIonChange={handleDoctorId}>
                      {doctorList &&
                        doctorList.map((doctor: any) => (
                          <IonSelectOption value={doctor.id} key={doctor.id}>
                            {doctor.name}
                          </IonSelectOption>
                        ))}
                    </IonSelect>
